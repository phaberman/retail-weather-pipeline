...
def load_city(city: str, bq_client: bigquery.Client, gcs_client: storage.Client) -> None:
    # Find the latest file for this city in GCS
    ...
    # Write to BigQuery — replace table on each load (idempotent)
    table_id = f"{GCP_PROJECT_ID}.{BQ_DATASET_RAW}.weather_{city}"
